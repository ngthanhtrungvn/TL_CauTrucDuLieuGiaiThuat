...
void main()
{
	...
	printf("Nhap so n: ");
	scanf("%d", &aa);
	createBTreeFromFile(bt.Root, file);
	displayBTree(bt.Root, aa);
	getch();
}/*End of main()*/
...
KeyStatus insertNode(NodePtr pCurrent, ItemType key, ItemType *upKey, NodePtr* newNode)
{
	...
	if (pos < numTree && strcmpi(key.eng , pCurrent->Keys[pos].eng))
		return Duplicate;
...
void displayBTree(NodePtr pRoot, int blanks) {
	if (pRoot) {
		...
		for (i = 0; i < pRoot->numTree; i++)
			printf("%s:%s ", pRoot->Keys[i].eng, pRoot->Keys[i].viet);
		printf("\n");
		for (i = 0; i <= pRoot->numTree; i++)
...
{
	int pos = 0;
	while (pos < numTree && strcmp(key.eng , keyArray[pos].eng))
		pos++;
	return pos;
...
NodePtr searchNode(NodePtr root, ItemType key)
{
	...
	while (pCurrent)
	{
		numTree = pCurrent->numTree;
		pos = searchPosition(key, pCurrent->Keys, numTree);
		if (pos < numTree && strcmp(key.eng , pCurrent->Keys[pos].eng))
		{
			return pCurrent;
		}
...
KeyStatus remove(NodePtr &root, NodePtr pCurrent, ItemType key)
{
	...
	if (Branch[0] == NULL)
	{
		if (pos == numTree || strcmp(key.eng , keyArray[pos].eng))
			return SearchFailure;
...
void createBTreeFromFile(BNode* &root, const char file[]) {
	FILE * f = fopen(file, "r+");
	int n;
	fscanf(f, "%d", &n);
	for (int i = 0; i < n; i++)
